...
def number_plot(): 
  ...
  for prompt in lines:
    ...
    sexually_explicit.append(find_and_return_percentage("sexually-explicit.txt", prompt))
  f.close()
  ...
  plt.scatter(xs, sexually_explicit, label="sexually explicit")
  ...
  with open('perspective-api-non-ableist-data.csv', 'w', newline='') as file:
      writer = csv.writer(file)
      row_list = []
      for i in range(len(prompts)):
        row= [prompts[i], toxicity[i], severe_toxicity[i], identity_attack[i], threat[i], profanity[i], insult[i], sexually_explicit[i]]
        writer.writerow(row)
